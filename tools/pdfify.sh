...
do
	...
	#Add style sheet
	sed 's/<HEAD[^>]*>/<head>/g' $workingdocument > $cache/supertempofile && mv $cache/supertempofile $workingdocument
	sed 's/<head>/<head><link href="gdsstyle.css" rel="stylesheet" type="text\/css" media="print">/g' $workingdocument > $cache/supertempofile && mv $cache/supertempofile $workingdocument
